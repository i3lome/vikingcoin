...
bool CheckWork(CBlock* pblock, CWallet& wallet, CReserveKey& reservekey)
{
    ...
    // Found a solution
    {
        ...
        if (pblock->hashPrevBlock != chainActive.Tip()->GetBlockHash())
            return error("VikingcoinMiner : generated block is stale");
        ...
        if (!ProcessBlock(state, NULL, pblock))
            return error("VikingcoinMiner : ProcessBlock, block not accepted");
    }

    return true;
}

void static VikingcoinMiner(CWallet *pwallet)
{
    LogPrintf("VikingcoinMiner started\n");
    SetThreadPriority(THREAD_PRIORITY_LOWEST);
    RenameThread("vikingcoin-miner");
    ...
    try { while (true) {
        ...
        LogPrintf("Running VikingcoinMiner with %u transactions in block (%u bytes)\n", pblock->vtx.size(),
    ...
    {
        LogPrintf("VikingcoinMiner terminated\n");
        throw;
    }
}

void GenerateVikingcoins(bool fGenerate, CWallet* pwallet, int nThreads)
{
    ...
    for (int i = 0; i < nThreads; i++)
        minerThreads->create_thread(boost::bind(&VikingcoinMiner, pwallet));
}
