...
void Shutdown()
{
    ...
    RenameThread("vikingcoin-shutoff");
    ...
    if (pwalletMain)
        bitdb.Flush(false);
    GenerateVikingcoins(false, NULL, 0);
#endif
    StopNode();
...
std::string HelpMessage(HelpMessageMode hmm)
{
    ...
    strUsage += "  -conf=<file>           " + _("Specify configuration file (default: vikingcoin.conf)") + "\n";
    if (hmm == HMM_VIKINGCOIND)
    {
#if !defined(WIN32)
    ...
    strUsage += "  -pid=<file>            " + _("Specify pid file (default: vikingcoind.pid)") + "\n";
    ...
    strUsage += "\n" + _("RPC SSL options: (see the Vikingcoin Wiki for SSL setup instructions)") + "\n";
...
{
    RenameThread("vikingcoin-loadblk");
...
#define PROCESS_DEP_ENABLE 0x00000001
...
#endif
    // Make sure only a single Vikingcoin process is using the data directory.
    boost::filesystem::path pathLockFile = GetDataDir() / ".lock";
    FILE* file = fopen(pathLockFile.string().c_str(), "a"); // empty lock file; created if it doesn't exist.
    if (file) fclose(file);
    static boost::interprocess::file_lock lock(pathLockFile.string().c_str());
    if (!lock.try_lock())
        return InitError(strprintf(_("Cannot obtain a lock on data directory %s. Vikingcoin Core is probably already running."), strDataDir));

    if (GetBoolArg("-shrinkdebugfile", !fDebug))
        ShrinkDebugFile();
    LogPrintf("\n\n\n\n\n\n\n\n\n\n\n\n\n\n\n\n\n\n\n\n");
    LogPrintf("Vikingcoin version %s (%s)\n", FormatFullVersion(), CLIENT_DATE);
    ...
    if (fDisableWallet) {
        ...
        if (nLoadWalletRet != DB_LOAD_OK)
        {
            ...
            else if (nLoadWalletRet == DB_NONCRITICAL_ERROR)
            {
                ...
                strErrors << _("Error loading wallet.dat: Wallet requires newer version of Vikingcoin") << "\n";
            else if (nLoadWalletRet == DB_NEED_REWRITE)
            {
                strErrors << _("Wallet needed to be rewritten: restart Vikingcoin to complete") << "\n";
    ...
    if (pwalletMain)
        GenerateVikingcoins(GetBoolArg("-gen", false), pwalletMain, GetArg("-genproclimit", -1));
#endif
