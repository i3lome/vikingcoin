...
// Copyright (c) 2013 Viking developers
...
const string strMessageMagic = "Viking Signed Message:\n";
...
static const int64 nStartSubsidy = 0.01 * COIN;
static const int64 nMinSubsidy = 0.00001066 * COIN;
...
bool CheckWork(CBlock* pblock, CWallet& wallet, CReserveKey& reservekey)
{
    ...
    // Found a solution
    {
        ...
        if (pblock->hashPrevBlock != hashBestChain)
            return error("VikingMiner : generated block is stale");
        ...
        if (!ProcessBlock(state, NULL, pblock))
            return error("VikingMiner : ProcessBlock, block not accepted");
    }
...
void static BitcoinMiner(CWallet *pwallet)
{
    ...
    try { loop {
        ...
        printf("Running VikingMiner with %"PRIszu" transactions in block (%u bytes)\n", pblock->vtx.size(),
